...
public class Solution {
    ...
    /**
     * Approach 1: Recursive Solution leveraging BST property (Most Elegant)
    ...
    /**
     * Approach 2: Iterative Solution leveraging BST property (Most Efficient)
     * Time Complexity: O(h) where h is the height of the tree
    ...
    /**
     * Approach 3: Path-based Solution (Works for any binary tree, not just BST)
     * Find paths from root to both nodes, then find the last common node
     * Time Complexity: O(h) where h is the height of the tree
     * Space Complexity: O(h) for the path lists
     */
    ...
    private boolean findPath(TreeNode root, TreeNode target, List<TreeNode> path) {
        ...
        // If target is not found in subtree rooted at current node, remove current node
        path.remove(path.size() - 1);
        return false;
    }
    
    /**
     * Approach 4: Optimized BST Solution with value comparison
     * Uses the fact that in BST, we can compare values directly
     * Time Complexity: O(h) where h is the height of the tree
    ...
    public TreeNode lowestCommonAncestorOptimized(TreeNode root, TreeNode p, TreeNode q) {
        ...
        while (current != null) {
            // If current value is greater than both p and q, go left
            if (current.val > q.val) {
                current = current.left;
            }
            // If current value is smaller than both p and q, go right
            else if (current.val < p.val) {
                current = current.right;
            }
            // If current value is between p and q (inclusive), it's the LCA
            else {
                return current;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        TreeNode result1_iter = solution.lowestCommonAncestorIterative(root, p1, q1);
        ...
        TreeNode result4 = solution.lowestCommonAncestorOptimized(root, p4, q4);
        ...
        TreeNode result5 = solution.lowestCommonAncestorPathBased(root, p5, q5);
